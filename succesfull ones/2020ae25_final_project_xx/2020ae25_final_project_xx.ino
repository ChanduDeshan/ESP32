  ...
  if (len % 10 == 0) {
    if (WiFi.status() == WL_CONNECTED) {      
      ...
      for (int i = 0; i < len; i++) {
        ...
        sprintf(httpRequestData, "api_key=Test8888&humidity=%2.2f",rec.humidity);
        http.POST(httpRequestData);
        http.addHeader("Content-Type", "application/x-www-form-urlencoded");        
        sprintf(httpRequestData, "api_key=Test8888&battery=%2.2f",rec.battery);
        http.POST(httpRequestData);
        http.addHeader("Content-Type", "application/x-www-form-urlencoded");        
        sprintf(httpRequestData, "api_key=Test8888&temperature=%2.2f",rec.temperature);
        http.POST(httpRequestData);
